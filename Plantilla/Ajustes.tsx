...
import { X, Key, ShieldCheck, RefreshCcw, Eye, EyeOff, ExternalLink, Lock, Database, Loader2, CheckCircle2, XCircle, Sparkles, Send, Table, Cpu, ChevronDown, Globe, Plus, Zap, Download, Upload } from 'lucide-react';
...
const CONFIG_KEYS = [
  'criptogo_portfolios', 
  'criptogo_real_assets', 
  'criptogo_favorites',
  'criptogo_currency', 
  'criptogo_market_mode', 
  'app_apikey',
  'app_selected_model', 
  'criptogo_dashboard_range', 
  'criptogo_allowed_ips',
  'app_memorized_ips_v2', 
  'guia_user_categories_v25',
  'app_current_view'
];
...
export const Ajustes: React.FC<AjustesProps> = ({ isOpen, onClose, apiKey, onApiKeySave, userIp }) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (typeof data !== 'object' || data === null || Array.isArray(data)) {
          throw new Error("Formato de archivo inválido. Debe ser un objeto JSON.");
        }
        
        // Validar que al menos tiene algunas llaves conocidas
        const hasKnownKeys = CONFIG_KEYS.some(k => k in data);
        if (!hasKnownKeys) {
          throw new Error("El archivo no contiene claves de configuración válidas para Cripto PRO.");
        }

        setIsImporting(true); // Activar modo recarga visual
        ...
        if (importedCount > 0) {
          // Feedback temporal para que el usuario vea la recarga
          // Se elimina el forzado a 'overview' para que el sistema mantenga la vista previa o la importada
          setTimeout(() => {
            window.location.reload();
          }, 2200);
        } else {
          setIsImporting(false);
          throw new Error("No se encontraron parámetros compatibles para importar.");
        }
      } catch (err: any) {
        setIsImporting(false);
        console.error("Import Error:", err);
        alert(`Error al importar: ${err.message || "El archivo Cripto.config no es válido."}`);
      } finally {
        if (fileInputRef.current) fileInputRef.current.value = '';
      }
    };
    reader.onerror = () => alert("Error al leer el archivo del dispositivo.");
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-300">
      ...
      {/* OVERLAY DE RECARGA SISTÉMICA */}
      {isImporting && (
        <div className="absolute inset-0 z-[130] bg-white flex flex-col items-center justify-center animate-in fade-in duration-500">
          <div className="p-8 flex flex-col items-center gap-6">
            <div className="relative">
              <RefreshCcw size={64} className="text-red-700 animate-spin" />
              <div className="absolute inset-0 flex items-center justify-center">
                <Database size={24} className="text-gray-900" />
              </div>
            </div>
            <div className="text-center space-y-2">
              <h4 className="text-2xl font-black text-gray-900 uppercase tracking-tighter">Sincronizando Base de Datos</h4>
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.3em] animate-pulse">Restaurando parámetros de sistema...</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl max-h-[95vh] overflow-hidden flex flex-col border border-gray-100 animate-in zoom-in-95">
        ...
        <div className="p-6 overflow-y-auto space-y-6 custom-scrollbar">
          ...
          {/* SECCIÓN 2: GESTIÓN DE DATOS (IMPORT/EXPORT) */}
          <section className="bg-gray-50/50 border border-gray-100 p-4 rounded-3xl space-y-4">
             <label className="text-[9px] font-black uppercase tracking-widest text-gray-900 flex items-center gap-2 px-1">
               <Database size={14} className="text-red-700" /> Respaldo y Restauración
             </label>
             <div className="grid grid-cols-2 gap-3">
                <button 
                  onClick={handleExportConfig}
                  className="flex items-center justify-center gap-3 p-3 bg-white border border-gray-200 text-gray-900 rounded-2xl hover:border-red-700 hover:text-red-700 transition-all group shadow-sm active:scale-95"
                >
                  <Download size={16} className="group-hover:translate-y-0.5 transition-transform" />
                  <span className="text-[10px] font-black uppercase tracking-widest">Exportar</span>
                </button>
                
                <label className="flex items-center justify-center gap-3 p-3 bg-white border border-gray-200 text-gray-900 rounded-2xl hover:border-red-700 hover:text-red-700 transition-all group shadow-sm active:scale-95 cursor-pointer">
             ...
             <p className="text-[8px] text-gray-400 font-bold uppercase tracking-tight text-center px-4">
               Guarda o recupera toda tu configuración local en un archivo Cripto.config
             </p>
          </section>
